...
\providecommand{\manualimage}[3][0.88\textwidth]{
\begin{figure}[H]
  \centering
...
角色 & 使用入口 & 主要使用内容 \\
\midrule
游客 & 游客端 React Web 页面 & 文本提问、语音提问、图片问答、多语讲解、路线推荐、回答来源查看、反馈提交。 \\
...
业务服务 & 启动 FastAPI、Vite 和 Ollama，并由 FastAPI 内部模块完成业务编排。 \\
数据库 & MySQL 保存运行时业务数据，Redis 保存缓存，MinIO 保存上传文件，Neo4j 保存文化关系镜像；PostgreSQL/pgvector 为后续增强预留。 \\
...
客户端 & 环境要求 \\
\midrule
游客端 & 支持现代移动浏览器和 PC 浏览器；语音功能需要麦克风权限，图片问答需要相机或相册权限。 \\
...
首次使用前，系统管理员应完成服务启动、数据库初始化、基础账号创建、模型服务配置、知识库初始化和访问域名配置。普通用户只需获得系统访问地址、账号密码和对应角色权限。若使用语音、拍照或文件上传功能，客户端首次访问时需要允许浏览器申请的权限。

\section{各客户端进入方式}

\subsection{项目入口页}

用户打开系统访问地址后进入项目介绍页。页面集中展示系统定位、可信 AI 链路和四类客户端入口，可分别进入游客端、学生端、导游端与管理端。当前演示环境保留角色入口与未登录状态提示，后续接入统一认证时可在入口处增加账号校验。

\manualimage[0.86\textwidth]{portal-overview.png}{项目介绍与四端入口页}

\subsection{游客端进入方式}

游客可通过游客端网址或指向该网址的景区二维码进入系统。免登录模式下可直接使用基础导览功能；需要保存历史记录、提交反馈或使用个性化路线时，系统提示游客登录或授权。游客端首页默认展示文本提问框、语音按钮、图片入口、语言选择和路线推荐入口。

\manualimage[0.86\textwidth]{tourist-home.png}{游客端首页}

\subsection{学生端进入方式}

学生使用学校或培训机构分配的账号登录。登录成功后进入实训中心首页，可查看可用训练场景、最近训练记录、待完成任务和评分报告入口。学生端入口一般位于系统首页的“AI 导游实训”或角色工作台中。

\manualimage[0.86\textwidth]{student-home.png}{学生端实训首页}

\subsection{导游端进入方式}

导游使用导游账号登录后进入协同工作台。系统根据景区、班次或权限范围展示游客会话列表。高风险会话、待人工接管会话和已接管会话会按状态分组展示。导游端支持移动设备快速查看，也可在 PC 端处理较复杂的修正内容。

\manualimage[0.86\textwidth]{guide-dashboard.png}{导游端协同首页}

\subsection{管理端进入方式}

管理员使用后台账号登录后进入管理端首页。管理端左侧或顶部显示用户管理、知识库管理、内容审核、术语维护、图谱维护、日志查询和系统状态等菜单。无权限菜单不会显示；直接访问无权限地址时，系统返回权限不足提示。

\manualimage[0.9\textwidth]{system-dashboard.png}{管理端首页}
...
\manualimage[0.86\textwidth]{tourist-chat.png}{游客文本问答页}

\subsection{语音问答}

\begin{enumerate}[leftmargin=2em]
  \item 在游客端首页点击语音按钮。
  \item 首次使用时允许浏览器访问麦克风。
...
\manualimage[0.86\textwidth]{tourist-voice.png}{游客语音问答页}
...
\manualimage[0.86\textwidth]{tourist-image.png}{游客图片问答页}
...
\manualimage[0.86\textwidth]{tourist-chat.png}{游客多语讲解页}
...
\manualimage[0.86\textwidth]{tourist-routes.png}{游客路线推荐页}
...
\manualimage[0.86\textwidth]{student-scenarios.png}{学生训练场景页}
...
\manualimage[0.86\textwidth]{student-training.png}{学生讲解提交页}
...
\manualimage[0.86\textwidth]{student-reports.png}{学生评分报告页}
...
\manualimage[0.86\textwidth]{student-reports.png}{学生训练历史页}
...
\manualimage[0.86\textwidth]{guide-sessions.png}{导游会话列表页}
...
\manualimage[0.86\textwidth]{guide-takeover.png}{导游会话接管页}
...
\manualimage[0.86\textwidth]{guide-corrections.png}{导游回答修正页}
...
\manualimage[0.9\textwidth]{system-users.png}{管理端用户权限页}
...
\manualimage[0.9\textwidth]{knowledge-documents.png}{管理端知识文档页}
...
\manualimage[0.9\textwidth]{knowledge-review.png}{管理端知识审核页}
...
\manualimage[0.9\textwidth]{knowledge-terms.png}{管理端术语维护页}
...
\manualimage[0.9\textwidth]{knowledge-graph.png}{管理端文化图谱页}
...
\manualimage[0.9\textwidth]{system-logs.png}{管理端日志查询页}
...
\manualimage[0.9\textwidth]{system-health.png}{管理端系统状态页}
...
游客端或学生端无法录音时，应检查浏览器是否授权麦克风权限。无法拍照或上传图片时，应检查相机、相册和文件权限。若权限已拒绝，可在浏览器设置或系统设置中重新开启。
