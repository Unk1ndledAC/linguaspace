...
系统客户端以 Web 端、H5 或小程序为主要使用入口，游客端可部署在移动终端，学生端、导游端和管理端可优先面向 PC 浏览器。服务端需要具备运行后端服务、数据库、缓存、向量数据库、图数据库和模型推理服务的基础资源。
...
MVP 阶段可采用 Docker Compose 在单机或少量服务器上部署。后端框架建议使用 FastAPI，业务数据库使用 PostgreSQL，向量数据库使用 Milvus，图数据库使用 Neo4j，缓存使用 Redis，文件存储使用 MinIO。后续如需支持更高并发，可扩展为 Kubernetes 部署方式。
...
\diagramimage{software-overall-process.png}{软件总体流程图}
