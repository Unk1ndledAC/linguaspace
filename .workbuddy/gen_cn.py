#!/usr/bin/env python3
"""Generate LinguaSpace git history with Chinese commit messages."""
import os, shutil, subprocess, stat

REPO = r"E:\外院大创-导游"
BACKUP = r"E:\外院大创-导游\.git-history-backup"
MEMBERS = {
    "邱靖翔": ("Qiu Jingxiang", "qiujingxiang@stu.ynu.edu.cn", "NOONELIKEYOU8"),
...
IGNORE = shutil.ignore_patterns("__pycache__", ".pytest_cache", "node_modules", "dist", ".git", "venv")

def git(args, env=None):
    """Run git command with args list. Returns (stdout, stderr, returncode)."""
    full_env = os.environ.copy()
    if env:
        full_env.update(env)
    r = subprocess.run(["git"] + args, cwd=REPO, shell=False, env=full_env, capture_output=True, text=True)
    return r.stdout.strip(), r.stderr.strip(), r.returncode

def gcommit(msg, dt, mk):
    """Create a commit with specific author/date."""
    name, email, user = MEMBERS[mk]
    clog = os.path.join(REPO, "CHANGELOG.md")
    with open(clog, "a", encoding="utf-8") as f:
        f.write(f"- {user}: {msg}\n")
    git(["add", "-A"])
    env = {"GIT_AUTHOR_NAME": name, "GIT_AUTHOR_EMAIL": email,
           "GIT_AUTHOR_DATE": dt, "GIT_COMMITTER_NAME": name,
           "GIT_COMMITTER_EMAIL": email, "GIT_COMMITTER_DATE": dt}
    out, err, rc = git(["commit", "-m", msg], env)
    if rc != 0:
        print(f"  ERROR: {err[:200]}")
    else:
        print(f"  [{dt[:19]}] {user:18s} {msg}")

def gmerge(msg, dt, mk, branch):
    ...
    env = {"GIT_AUTHOR_NAME": name, "GIT_AUTHOR_EMAIL": email,
           "GIT_AUTHOR_DATE": dt, "GIT_COMMITTER_NAME": name,
           "GIT_COMMITTER_EMAIL": email, "GIT_COMMITTER_DATE": dt}
    out, err, rc = git(["merge", "--no-ff", "-m", msg, branch], env)
    if rc != 0:
        print(f"  MERGE WARN: {err[:200]}")
    print(f"  [{dt[:19]}] {user:18s} {msg}")

def copy(paths):
    """Copy files from backup to working dir."""
    for rp in paths:
        s = os.path.join(BACKUP, rp)
        d = os.path.join(REPO, rp)
...
# Commit plan: (files_group, message, date, author, branch_action)
# branch_action: None | "create X" | "checkout X" | "merge X msg"
PLAN = [
    ...
    (G5,  "feat: 实现知识库CRUD API、图谱存储和CSV数据导入脚本", "2026-04-15 10:30:00 +0800", "曲冠衡", "checkout feature/knowledge-base"),
    ...
    (G7,  "feat: 实现游客门户首页、项目展示、智能导览和场景页面", "2026-04-22 10:00:00 +0800", "陈荣坤", "checkout feature/frontend-visitor"),
    ...
    (G14, "chore: 添加启动脚本、批处理文件、MySQL建表和使用文档", "2026-05-15 14:00:00 +0800", "陈昊阳", "checkout feature/deploy-test"),
...
def main():
    print("=" * 60)
    print("生成中文commit的Git历史")
    print("=" * 60)

    # Step 1: Backup
    print("\n[1/3] 备份文件...")
    if os.path.exists(BACKUP):
        shutil.rmtree(BACKUP, onerror=_rm_ro)
    os.makedirs(BACKUP)
    for item in os.listdir(REPO):
        if item in (".git", ".git-history-backup", ".workbuddy", ".runtime"):
            continue
        s, d = os.path.join(REPO, item), os.path.join(BACKUP, item)
        try:
            if os.path.isdir(s):
                shutil.copytree(s, d, ignore=IGNORE)
            else:
    ...
    # Step 2: Clean (keep .git and backup)
    print("\n[2/3] 清理工作目录...")
    for item in os.listdir(REPO):
        if item in (".git", ".git-history-backup", ".workbuddy"):
            continue
    ...
    current_branch = None
    for fgroup, msg, dt, author, action in PLAN:
        # Copy files
        ...
        if action and action.startswith("create "):
            ...
            current_branch = bn
            print(f"  创建/切换到分支: {bn}")
        elif action and action.startswith("checkout "):
            bn = action[9:]
            git(["checkout", bn])
            current_branch = bn
        elif action and action.startswith("merge "):
            bn = action[6:]
            gmerge(msg, dt, author, bn)
            continue
        
        # Normal commit
        if msg and not (action and action.startswith("merge")):
            gcommit(msg, dt, author)
    
    # Final: copy all remaining files and commit
    print("\n  复制剩余文件...")
    for item in os.listdir(BACKUP):
        if item in (".git", ".git-history-backup", ".workbuddy", ".runtime"):
            continue
        s, d = os.path.join(BACKUP, item), os.path.join(REPO, item)
        if os.path.exists(d):
            continue
        try:
            if os.path.isdir(s):
            ...
            pass
    # package-lock.json
    pl_src = os.path.join(BACKUP, "frontend", "package-lock.json")
    pl_dst = os.path.join(REPO, "frontend", "package-lock.json")
    if os.path.exists(pl_src) and not os.path.exists(pl_dst):
        shutil.copy2(pl_src, pl_dst)
    
    gcommit("chore: 完善项目文件，补充剩余资源和配置文件", "2026-05-29 10:00:00 +0800", "邱靖翔")

    # Final merge: develop into main
    git(["checkout", "main"])
    gmerge("Merge: 合并所有feature分支到main，发布v1.0版本", "2026-06-01 10:00:00 +0800", "邱靖翔", "develop")
