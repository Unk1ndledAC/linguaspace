import { HeartHandshake, History, Map, Route, Send, Sparkles, Compass, BookOpen, Camera } from "lucide-react";
...
export function TouristCultureTipsPage() {
  ...
  return (
    <section className="page-stack tourist-service-page tourist-culture-page">
      <section className="scenic-banner culture-banner">
        <div>
          <span className="page-kicker"><HeartHandshake size={16} /> CULTURE TIPS</span>
          <h1>带着尊重走近当地文化</h1>
          <p>文化礼仪、禁忌和风险提示均来自审核知识库。</p>
        </div>
      </section>
      {error && <ErrorState message={error} />}{!error && !items.length && <EmptyState label="暂无命中的审核文化提示" />}{!!items.length && <section className="case-grid">{items.map((item) => <article key={item.id}><HeartHandshake size={20} /><h3>{item.title}</h3><p>{item.summary}</p><small>{item.tags.join(" · ")}</small></article>)}</section>}
    </section>
  );
}
